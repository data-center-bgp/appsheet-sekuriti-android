...
export default function OrangKeluarList({ navigation }: { navigation: any }) {
  const [orangKeluar, setOrangKeluar] = useState<OrangKeluarItem[]>([]);
  ...
  const fetchData = async (
    ...
    try {
      ...
      if (result) {
        ...
        if (replace || page === 1) {
          setOrangKeluar(result);
        } else {
          setOrangKeluar((prev) => [...prev, ...result]);
        }
      }
    } catch (err) {
      console.error("Error fetching orang keluar:", err);
  ...
  const deleteItem = async (item: OrangKeluarItem) => {
    Alert.alert(
      "Konfirmasi Hapus",
  ...
  const renderItem = (item: OrangKeluarItem, index: number) => {
      ...
      >
        ...
        <View style={styles.statsRow}>
          <View style={styles.statItem}>
            <Icon name="user-minus" type="feather" size={16} color="#dc3545" />
            <Text style={styles.statText}>Orang Keluar</Text>
          </View>
        ...
        <View style={styles.cardFooter}>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate("OrangKeluarCreate", { editData: item })
            }
          >
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerStats}>
          <Text style={styles.headerSubtitle}>
            {orangKeluar.length} dari {totalItems}{" "}
            {totalItems === 1 ? "entry" : "entries"}
          </Text>
          <View style={styles.totalStats}>
            <Icon name="user-minus" type="feather" size={12} color="#dc3545" />
            <Text style={styles.totalStatsText}>
              {orangKeluar.length} people on current page
            </Text>
          </View>
        </View>
      </View>
      ...
      >
        {loading ? (
          ...
          <View style={styles.centerContainer}>
            <Icon name="user-minus" type="feather" size={64} color="#6c757d" />
            ...
            <Text style={styles.emptySubtitle}>
              {searchQuery
                ...
                : "Tambahkan data orang keluar pertama Anda"}
            </Text>
            {!searchQuery && (
              <Button
                title="Tambah Data"
          ...
          <View style={styles.listContainer}>
            {orangKeluar.map((item, index) => renderItem(item, index))}
            {renderPagination()}
          </View>
        )}
      </ScrollView>
