...
export default function SuratMasukList({ navigation }: { navigation: any }) {
  ...
  const fetchData = async (
    ...
    try {
      ...
      if (searchQuery.trim()) {
        query = query.or(
          `ID.ilike.%${searchQuery}%,nama_pengirim.ilike.%${searchQuery}%,nama_penerima.ilike.%${searchQuery}%,jenis_surat.ilike.%${searchQuery}%,keterangan.ilike.%${searchQuery}%,sekuriti.ilike.%${searchQuery}%,pos.ilike.%${searchQuery}%,business_unit.ilike.%${searchQuery}%`
        );
      }
      ...
      if (result) {
        ...
        if (replace || page === 1) {
          ...
          setSuratMasuk((prev) => [...prev, ...result]);
        }
      }
    } catch (err) {
  ...
  const deleteItem = async (item: SuratMasukItem) => {
    Alert.alert(
      "Konfirmasi Hapus",
  ...
  const renderItem = (item: SuratMasukItem, index: number) => {
      ...
      >
        ...
        <View style={styles.statsRow}>
          <View style={styles.statItem}>
            <Icon name="mail" type="feather" size={16} color="#007bff" />
            <Text style={styles.statText}>Surat Masuk</Text>
          </View>
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.infoRow}>
            <View style={styles.infoItem}>
              <Icon name="tag" type="feather" size={14} color="#495057" />
        ...
        <View style={styles.cardFooter}>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate("SuratMasukCreate", { editData: item })
            }
          >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerStats}>
          <Text style={styles.headerSubtitle}>
            {suratMasuk.length} dari {totalItems}{" "}
            {totalItems === 1 ? "document" : "documents"}
          </Text>
          <View style={styles.totalStats}>
            <Icon name="mail" type="feather" size={12} color="#007bff" />
            <Text style={styles.totalStatsText}>
              {suratMasuk.length} documents on current page
            </Text>
          </View>
        </View>
      </View>
      ...
      {/* Search Bar */}
      <SearchBar
        placeholder="Cari berdasarkan ID, pengirim, penerima, jenis surat, business unit..."
      ...
      >
        {loading ? (
          ...
          <View style={styles.centerContainer}>
            ...
            <Text style={styles.emptySubtitle}>
              {searchQuery
                ...
                : "Tambahkan surat masuk pertama Anda"}
            </Text>
            {!searchQuery && (
              <Button
                title="Tambah Surat"
          ...
          <View style={styles.listContainer}>
            {suratMasuk.map((item, index) => renderItem(item, index))}
            {renderPagination()}
          </View>
        )}
      </ScrollView>
