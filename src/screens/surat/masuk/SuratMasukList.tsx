...
export default function SuratMasukList({ navigation }: { navigation: any }) {
  ...
  async function fetchSuratMasuk() {
    try {
      ...
      const { data: surat_masuk, error } = await query;
  ...
  const filteredData = suratMasuk.filter((item) => {
    ...
    return (
      ...
      (item.jenis_surat && item.jenis_surat.toLowerCase().includes(query)) ||
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerStats}>
          <Text style={styles.headerSubtitle}>
            {filteredData.length}{" "}
            {filteredData.length === 1 ? "document" : "documents"}
          </Text>
