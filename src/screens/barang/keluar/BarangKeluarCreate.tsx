...
export default function BarangKeluarCreate() {
  ...
  const [detailItems, setDetailItems] = useState<DetailDoKeluar[]>([
    {
      serial_number: "",
      nama_barang: "",
      jumlah: 0,
  ...
  // Photo handling with Supabase Storage
  ...
  const loadExistingPhotos = async () => {
    ...
    try {
      const { data: existingPhotos, error } = await supabase
        .from("foto_do_keluar")
        .select("*")
        .eq("barang_keluar_id", editData.id);
      ...
      { text: "Batal", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          if (photo.uploaded && photo.storage_path) {
            ...
            if (photo.id) {
              await supabase.from("foto_do_keluar").delete().eq("id", photo.id);
            }
          }
  ...
  const uploadAllPhotos = async (barangKeluarId: string): Promise<boolean> => {
  ...
  const validateForm = () => {
    ...
    if (!formData.kurir.trim()) {
      errors.kurir = "Nama kurir wajib diisi";
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editData) {
        ...
        barangKeluarId = formData.id!;

        // Delete existing details
        await supabase
          .from("detail_do_keluar")
          .delete()
          .eq("barang_keluar_id", barangKeluarId);
      } else {
        // Insert main record
        const dataToInsert = {
          ...formData,
      ...
      // Upload new photos
      const photoUploadSuccess = await uploadAllPhotos(barangKeluarId);
  ...
  const renderDetailItem = (item: DetailDoKeluar, index: number) => (
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header
        centerComponent={{
          text: editData ? "Edit Barang Keluar" : "Tambah Barang Keluar",
          style: { color: "white", fontSize: 18, fontWeight: "bold" },
        }}
        leftComponent={{
          icon: "arrow-left",
        ...
        >
          ...
          <View style={styles.formHeader}>
            ...
            <Text style={styles.formTitle}>
              {editData ? "Edit Data Barang Keluar" : "Data Barang Keluar Baru"}
            </Text>
            <Text style={styles.formSubtitle}>
              Lengkapi informasi barang yang keluar dari area
            </Text>
            {/* Business Unit Info */}
          ...
          <Card containerStyle={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <Text style={styles.cardTitle}>Informasi Personel & Tujuan</Text>
            </View>

            <Input
              placeholder="Nama lengkap kurir/pembawa"
              label="Nama Kurir *"
              value={formData.kurir}
              onChangeText={(text) => {
                setFormData({ ...formData, kurir: text });
                if (validationErrors.kurir) {
                  setValidationErrors({
                    ...validationErrors,
                    kurir: "",
                  });
              ...
              errorMessage={validationErrors.kurir}
              leftIcon={{
                name: "user",
                type: "feather",
                size: 20,
